...
class MainActivity : FragmentActivity() {
    ...
    private fun scheduleAttendanceReminder() {
        // BUG FIX: Previously created a bare PeriodicWorkRequest without setInitialDelay,
        // which caused the worker to fire immediately at an arbitrary time on first run
        // instead of at the user's preferred reminder time (default 9:00 PM).
        // ReminderScheduler.schedule() correctly calculates the delay until the next
        // occurrence of the target time and uses ExistingPeriodicWorkPolicy.UPDATE
        // so user-preference changes in Settings also take effect immediately.
