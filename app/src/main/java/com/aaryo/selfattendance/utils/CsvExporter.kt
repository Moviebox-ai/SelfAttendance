...
object CsvExporter {
    ...
    fun export(
        ...
        try {
            ...
            // BUG FIX #5: Use a date-stamped filename so each export is unique
            // and the previous file is not silently overwritten.
            val dateStamp = LocalDate.now().format(DateTimeFormatter.ofPattern("yyyy-MM-dd"))
            val fileName = "attendance_report_$dateStamp.csv"
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
                ...
                uri?.let { fileUri ->
                    context.contentResolver.openOutputStream(fileUri)?.use { stream ->
                        stream.write(csvContent.toByteArray(Charsets.UTF_8)) // BUG FIX: explicit UTF-8 prevents corruption on non-ASCII data
                        stream.flush()
                    }
                    Toast.makeText(
                        context,
                ...
                // BUG FIX: Specify UTF-8 explicitly. writeText() without a charset
                // uses the platform default encoding (often not UTF-8 on older
                // Android devices), which corrupts non-ASCII characters such as
                // names in Hindi/Gujarati or other local scripts.
