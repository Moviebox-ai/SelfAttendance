...
object SalarySlipGenerator {
    ...
    // ── Colors ───────────────────────────────────────────────────────────────────
    private val COLOR_PRIMARY   = Color.parseColor("#FFB300")   // Amber
    private val COLOR_LIGHT     = Color.parseColor("#FFF8E1")   // warm amber light
    ...
    fun generate(
        ...
        val currencySymbol = CurrencyManager.getSymbol(PreferencesManager(context).selectedCurrency)
        executor.execute {
            try {
                val pdf = PdfDocument()
                ...
                drawSlip(canvas, profile, attendanceList, month, bonus, deductions, currencySymbol)
    ...
    private fun drawSlip(
        ...
        fun formatMoney(amount: Double): String = formatMoney(amount, currencySymbol)
        ...
        p.color = Color.parseColor("#FFE0B2")
        ...
        val holiday = list.count { it.status == "HOLIDAY" }
        val absent  = list.count { it.status == "ABSENT" }
        val overtime = list.sumOf { it.overtimeHours }
        val totalDays = present + half + holiday + absent

        // 5 stat boxes
        val boxW = (PAGE_W - 80f - 32f) / 5f
        val colorPurple = Color.parseColor("#7C4DFF")
        val boxes = listOf(
            Triple("Present", "$present d", COLOR_GREEN),
            Triple("Half Day", "$half d", COLOR_AMBER),
            Triple("Holiday", "$holiday d", colorPurple),
            Triple("Absent", "$absent d", COLOR_RED),
            Triple("Overtime", "${String.format(java.util.Locale.US, "%.1f", overtime)}h", COLOR_PRIMARY)
        )
        boxes.forEachIndexed { i, (label, value, color) ->
            ...
            p.textSize = 16f
            p.isFakeBoldText = true
            c.drawText(value, bx + 10f, y + 28f, p)

            p.color = COLOR_MUTED
            p.textSize = 10f
            p.isFakeBoldText = false
            c.drawText(label, bx + 10f, y + 46f, p)
        }
        ...
        if (holiday > 0) {
            rows.add(listOf("Paid Holiday", "$holiday × ${formatMoney(perDay)}", formatMoney(holidayPay)))
        }
        rows.add(listOf("Half Day Pay", "$half × ${formatMoney(perDay / 2)}", formatMoney(halfPay)))
        rows.add(listOf("Overtime Pay", "${String.format(java.util.Locale.US, "%.1f", overtime)}h × ${formatMoney(profile.overtimeRate)}", formatMoney(overtimePay)))
        rows.add(listOf("Bonus", "–", formatMoney(bonus)))
        ...
        val gross = basicPay + holidayPay + halfPay + overtimePay + bonus
        ...
        // ── Deductions table ──────────────────────────────────────────────────
        // excluded from basicPay (gross only counts PRESENT + HALF days). Adding
        // absentDed here was double-penalising the employee: they got no pay for
        // absent days AND had their already-not-paid absent days deducted again.
        // This made the PDF show a much lower net than the dashboard.
        // Only "other deductions" (e.g. TDS, advances) are legitimate deductions here.
        ...
        p.textSize = 11f
        p.isFakeBoldText = false
        c.drawText("Earned: ${String.format(java.util.Locale.US, "%.1f", earnedPct)}%", PAGE_W - 160f, y + 26f, p)

        p.color = Color.parseColor("#FFCC80")
        p.textSize = 11f
    ...
    private fun formatMoney(amount: Double, symbol: String = "₹"): String {
        ...
        val prefix = if (amount < 0) "-$symbol" else symbol
