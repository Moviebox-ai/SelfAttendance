...
object ReferralRepository {
    ...
    private suspend fun awardReferralCoinsToReferrer(referrerUid: String, referredUid: String) {
        try {
            ...
            db.runTransaction { transaction ->
                ...
                transaction.set(
                    userDocRef,
                    mapOf("rewards" to mapOf(
                        "coinBalance"      to currentBal + REFERRAL_REWARD_COINS,
                        "totalCoinsEarned" to currentTotal + REFERRAL_REWARD_COINS,
    ...
    // BUG FIX: Now uses a Firestore transaction per eligible referral to prevent
    ...
    suspend fun collectPendingReferralRewards(): Int {
        ...
        try {
            ...
            // BUG FIX: Process each eligible referral inside its own Firestore
                ...
                try {
                    db.runTransaction { transaction ->
                        ...
                        transaction.set(
                            userDocRef,
                            mapOf("rewards" to mapOf(
                                "coinBalance"      to currentBalance + REFERRAL_REWARD_COINS,
                                "totalCoinsEarned" to currentTotal   + REFERRAL_REWARD_COINS,
                                "updatedAt"        to System.currentTimeMillis()
                            )),
                            SetOptions.merge()
