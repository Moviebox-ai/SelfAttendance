...
class StaffRepository(context: Context) {
     ...
     * - Present days, Half days (0.5), Overtime hours
     * - Outstanding Advances taken during or before that month
     */
    suspend fun calculateEmployeeMonthlySalary(
        ...
        val daysInMonth = ym.lengthOfMonth().toDouble()

        val grossSalary: Double = when (employee.salaryType) {
            Employee.SALARY_TYPE_MONTHLY -> {
                val dailyRate = if (daysInMonth > 0) employee.baseSalary / daysInMonth else 0.0
                val workedDaysEquivalent = presentDays + (halfDays * 0.5) + paidLeaveDays
                val baseEarned = workedDaysEquivalent * dailyRate
                val otEarned = totalOvertimeHours * employee.overtimeRatePerHour
                baseEarned + otEarned
            }
            Employee.SALARY_TYPE_DAILY -> {
                val workedDaysEquivalent = presentDays + (halfDays * 0.5) + paidLeaveDays
                val baseEarned = workedDaysEquivalent * employee.baseSalary
                val otEarned = totalOvertimeHours * employee.overtimeRatePerHour
                baseEarned + otEarned
            }
            Employee.SALARY_TYPE_HOURLY -> {
                val regularEarned = (presentDays * 8.0 + halfDays * 4.0) * employee.hourlyRate
                val otEarned = totalOvertimeHours * (if (employee.overtimeRatePerHour > 0) employee.overtimeRatePerHour else employee.hourlyRate * 1.5)
                regularEarned + otEarned
            }
            else -> employee.baseSalary
        }

        // Sum advances that are pending for this month
        ...
        val otherDeductions = existingPayout?.otherDeductions ?: 0.0
