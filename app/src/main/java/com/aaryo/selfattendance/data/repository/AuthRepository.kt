...
class AuthRepository {
    ...
    suspend fun register(
        ...
        return try {
            ...
            // Generate the 6-digit unique ID as soon as the account exists,
    ...
    suspend fun firebaseAuthWithGoogle(
        ...
        return try {
            ...
            // Brand-new Google account -> mint its 6-digit unique ID right away.
    ...
    suspend fun saveUserProfile(
        ...
        return try {
            ...
            // BUG FIX: writing the full `profile` POJO via merge() also writes
     ...
     *
     * BUG FIX: EditProfileScreen showed a permanent "Generating your account
    ...
    /**
     * Ensures the given user has a 6-digit unique ID, generating one if
     * missing. Called after every successful login/registration so both
    ...
    // BUG FIX #4: All sub-collection deletes now use Firestore batch writes
    ...
    private suspend fun deleteUserFirestoreData(uid: String) {
        try {
            ...
            // BUG FIX: Also delete referral data on account deletion.
