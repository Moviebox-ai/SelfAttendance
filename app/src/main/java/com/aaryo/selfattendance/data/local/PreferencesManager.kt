...
class PreferencesManager(val context: Context) {
    ...
    /** Total count of times the user has marked attendance across all sessions. */
    var attendanceMarksCount: Int
        get() = prefs.getInt("pref_attendance_marks_count", 0)
        set(value) = prefs.edit().putInt("pref_attendance_marks_count", value).apply()

    fun incrementAttendanceMarksCount(): Int {
        val next = attendanceMarksCount + 1
        attendanceMarksCount = next
        return next
    }

    /** Total count of salary slips generated by the user. */
    var salarySlipGeneratedCount: Int
        get() = prefs.getInt("pref_salary_slip_gen_count", 0)
        set(value) = prefs.edit().putInt("pref_salary_slip_gen_count", value).apply()

    fun incrementSalarySlipGeneratedCount(): Int {
        val next = salarySlipGeneratedCount + 1
        salarySlipGeneratedCount = next
        return next
    }

    /** Timestamp (epoch ms) of the last in-app review prompt shown. */
    var lastReviewPromptTimeMs: Long
        get() = prefs.getLong("pref_last_review_prompt_ms", 0L)
        set(value) = prefs.edit().putLong("pref_last_review_prompt_ms", value).apply()

    /** Number of times the review dialog has been prompted to the user. */
    var reviewPromptCount: Int
        get() = prefs.getInt("pref_review_prompt_count", 0)
        set(value) = prefs.edit().putInt("pref_review_prompt_count", value).apply()

    /** Flag indicating user has interacted with or confirmed rating the app. */
    var hasUserReviewedApp: Boolean
        get() = prefs.getBoolean("pref_has_user_reviewed", false)
        set(value) = prefs.edit().putBoolean("pref_has_user_reviewed", value).apply()
}
