            ...
            try {
                ...
                repo.getProfile(user.uid).fold(
                    onSuccess = { profile ->
                        ...
                        // BUG FIX (new accounts skipping Setup Profile / getting
                        // bounced straight back to dashboard): ensureUniqueId()
                        // always creates a bare { uid, uniqueId } doc in
                        // users/{uid}, so doc.toObject() never returns null even
                        // for a brand-new account -- `profile != null` was always
                        // true. A profile only really "exists" once the user has
                        // filled the setup form, so require the one required
                        // field (`name`) to be non-blank too.
                        _profileExists.value = profile != null && profile.name.isNotBlank()
                    },
                    onFailure = { throwable ->
