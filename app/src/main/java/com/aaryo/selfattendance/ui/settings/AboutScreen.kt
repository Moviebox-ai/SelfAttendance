...
fun AboutScreen(navController: NavController) {
    ...
    AdsController.BannerAd()
        ...
        Column(
            ...
            // ── Footer — secret admin trigger (7 taps) ──────────────────
            SecretAdminTrigger(
                muted         = muted,
                onAdminAccess = { navController.navigate(Routes.ADMIN_PIN) }
            )
        }
    }
}
...
// ─────────────────────────────────────────────
//  Secret Admin Trigger — 7 taps on version text
//  Looks exactly like a normal footer.
//  No toast, no indication to normal users.
// ─────────────────────────────────────────────

@Composable
private fun SecretAdminTrigger(muted: Color, onAdminAccess: () -> Unit) {
    ...
    Text(
        ...
        modifier  = Modifier
            ...
            .clickable(
                ...
                if (now - lastTapMs > 3000) tapCount = 0
                lastTapMs = now
                tapCount++
                if (tapCount >= 7) {
                    tapCount = 0
...
private fun FeaturesList(
    ...
    val green  = Color(0xFF00C853)   // EmeraldGreen  — wallet exact
    val yellow = Color(0xFFFFB300)   // CoinGold      — wallet exact
    val blue   = Color(0xFF1565C0)   // SapphireBlue  — wallet exact
    val pink   = Color(0xFFE53935)   // CoralRed      — wallet exact
        ...
        items.forEachIndexed { idx, (title, desc) ->
            Row(
                ...
                Icon(
                    ...
                    contentDescription = null,
                    tint = Color(0xFF00C853),   // EmeraldGreen
                    modifier = Modifier.size(18.dp)
                )
            }
