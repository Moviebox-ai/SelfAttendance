...
fun SelfAttendanceTheme(
    ...
    if (!view.isInEditMode) {
        SideEffect {
            ...
            // now that MainActivity calls enableEdgeToEdge() — the system draws
            // behind the status bar and app content supplies its own background,
            // so an explicit status bar color is no longer needed or supported.
            // Only the (non-deprecated) icon-appearance flag is still set here.
