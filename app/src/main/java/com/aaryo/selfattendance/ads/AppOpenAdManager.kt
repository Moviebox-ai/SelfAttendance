...
class AppOpenAdManager(private val application: Application) :
    ...
    private val adUnitId: String
        get() = if (BuildConfig.DEBUG)
            ...
            "ca-app-pub-5703232582358249/3008194873"
    ...
    fun showAdIfAvailable() {
        ...
        // BUG FIX: showAdIfAvailable() is the single real gate every path (onStart
        // background→foreground, showOnColdStart's immediate show, and the
        // onAdLoaded "pending show" callback) funnels through to actually show the
        // App Open ad. None of those callers checked AdsController.isAdFreeActive,
        // so a user who just watched a rewarded ad to earn 30 minutes ad-free would
        // still get a full-screen App Open ad the moment they backgrounded and
        // reopened the app (or on the very next cold start). Checking it here closes
        // the gap for every caller at once instead of duplicating it in each one.
        if (AdsController.isAdFreeActive) {
            Log.d(TAG, "showAdIfAvailable: ad-free window active — skipping")
            return
        }
        if (!isAdAvailable()) {
