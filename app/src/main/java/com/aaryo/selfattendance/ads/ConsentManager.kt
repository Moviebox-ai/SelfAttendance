...
object ConsentManager {
    ...
    private fun initMobileAdsIfNeeded(
        ...
        if (mobileAdsInitialized) {
            Log.d(TAG, "MobileAds already initialized — skipping")
            onDone()
            return
        }
        ...
        com.google.android.gms.ads.MobileAds.initialize(activity.applicationContext) {
            ...
            Log.d(TAG, "✅ MobileAds initialized after consent")
            onDone()
        }
    }

    // ✅ FIX: UNKNOWN bhi allow karo — India mein yahi status aata hai
